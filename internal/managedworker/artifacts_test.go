...
func TestTaskArtifactsAcrossRealExecutions(t *testing.T) {
	dir := t.TempDir()
...
[workflows.deliver]
steps=[{command="plan",required_outputs=["spec.md"]},{command="build",inputs={spec="plan/spec.md"},required_outputs=["result.txt"]}]
`)
	write(filepath.Join(dir, "plan.md"), "{{task.spec}}")
	write(filepath.Join(dir, "build.md"), "{{inputs.spec}}")
...
`)
	write(filepath.Join(dir, "plan.md"), "{{task.spec}}")
	write(filepath.Join(dir, "build.md"), "{{task.output_dir}}/spec.md")
	write(filepath.Join(repo, "plan.sh"), `#!/bin/sh
set -eu
