...
export function Analytics({ jobs, loaded, error }) {
  ...
  return <div className="mx-auto max-w-[1500px] space-y-6 p-4 sm:p-6 lg:p-8">
    ...
    {view.kind === "error" ? <div role="alert" className="rounded-md border border-danger/35 bg-danger/10 px-3 py-2 text-sm text-danger">{view.message}</div> : view.kind === "loading" ? <Card className="grid place-items-center p-12 text-sm text-muted-foreground" role="status">Loading analytics…</Card> : <Card className="overflow-hidden">
      ...
      {view.kind === "ready" ? view.runs.map((run) => <div key={run.id} className="grid gap-2 border-b border-border px-4 py-3.5 last:border-b-0 sm:grid-cols-[minmax(8rem,1fr)_minmax(8rem,1fr)_10rem_12rem] sm:items-center sm:gap-4">
